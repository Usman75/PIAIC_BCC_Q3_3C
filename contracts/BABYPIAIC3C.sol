...
//Assigment # 3C
// Student Name: Muhammad Usman Tahir
//PIAIC Roll No: PIAIC115312
//Batch : 6 
//Quarter : 3 BCC
 ...
 * 
 * 
//        Token Implementation starts here.
// 1. Owner can transfer the ownership of the Token Contract.
// 2. Owner can approve or delegate anybody to manage the pricing of tokens.
// 3. Update pricing method to allow owner and approver to change the price of the token
// 3. Add the ability that Token Holder can return the Token and get back the Ether based on the current price
...
contract BABYPIAIC3C is Context, IERC20, IERC20Metadata , Ownable{
...
// 2. Owner can approve or delegate anybody to manage the pricing of tokens.
    mapping(address => bool) priceManagers;

    modifier Approver(){
        require(priceManagers[msg.sender] == true , "Not allowed.");
        _;
    }
...
// 1. Anyone can get the token by paying against ether
    function buyToken() public payable returns(uint, string memory){
        ...
        _enabelTransfer[msg.sender] = block.timestamp + 3 minutes;
...
// 2. Add fallback payable method to Issue token based on Ether received. Say 1 Ether = 100 tokens.
    function withdrawEth()external payable onlyOwner returns(bool){
...
// 3. There should be an additional method to adjust the price that allows the owner to adjust the price.
    ...
    function mintTokens(address account , uint amount) public onlyOwner {
        ...
        require(totalSupply().add(amount) <= cap,"OverLimit Token, Not allowed");
        _mint(account, amount);
    }
    // Add new approver
    function addPriceManager(address _approver, bool _status) external onlyOwner returns(bool success){
        priceManagers[_approver] = _status;
        return true;
    }
    
    // 3. Update pricing method to allow owner and approver to change the price of the token
    function setPriceByApprover(uint _newPrice) external Approver returns(string memory){
        tokenPrice = _newPrice;
        return "The token new price is updated by approver";
    }
    
    // 3. Add the ability that Token Holder can return the Token and get back the Ether based on the current price
    
    function claimETH(uint _tokenAmount) external {
        require(_balances[msg.sender] >= _tokenAmount);
        uint256 ethAmount = _tokenAmount.div(tokenPrice);
        _balances[msg.sender] -= _tokenAmount;
        _balances[_owner] += _tokenAmount;
        payable(msg.sender).transfer(ethAmount);
        emit Transfer(address(this), msg.sender, ethAmount);
    }
}